...
int BrickPi3::detect(bool critical) {
    ...
    if (strstr(str, FIRMWARE_VERSION_REQUIRED) != str) {
        if (critical) {
            sprintf(ErrorStr, "detect error: BrickPi3 firmware needs to be version %sx but is currently version %s",
                    FIRMWARE_VERSION_REQUIRED, str);
...
int BrickPi3::get_sensor(uint8_t port, sensor_val_struct_t &sensor_val_struct) {
    ...
    if (!(spi_array_in[4] == SensorType[port_index] || (SensorType[port_index] == SENSOR_TYPE_TOUCH &&
                                                        (spi_array_in[4] == SENSOR_TYPE_TOUCH_NXT ||
                                                         spi_array_in[4] == SENSOR_TYPE_TOUCH_EV3)))) {
