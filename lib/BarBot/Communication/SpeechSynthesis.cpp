...
void SpeechSynthesis::speak(std::string text) {
    ...
    std::string test = "mplayer -ao alsa -noconsolecontrols -volume 10000 \"http://translate.google.com/translate_tts?ie=UTF-8&client=tw-ob&q=" + text + "&tl=nl\" > /dev/null 2>&1";
    Logger::log(TAG, text);
    system(test.c_str());
}
