...
async function get_locations() {
    try {
        let {rows} = await client.query("SELECT * FROM location;");
        return rows;
    } catch (e) {
        return e;
    }
...
async function get_app_requests() {
    try {
        let {rows} = await client.query("SELECT * FROM app_request WHERE handled = false ORDER BY time ASC;");
        return rows;
    } catch (e) {
        return e;
    }
...
// Socket Code
{
    ...
    function location_map(loc) {
        return [loc.location, loc.node, loc.side];
    }

    function app_request_map(req) {
        return [req.name, req.location];
    }
    ...
    async function handle_command(command) {
        ...
        } else if(command[0] === "get_locations") {
            let locations = await get_locations();
            return locations.map(location_map).flat();
        } else if(command[0] === "get_app_requests") {
            let locations = await get_app_requests();
            return locations.map(app_request_map).flat();
        }
